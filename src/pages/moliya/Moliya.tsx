...
export function Moliya() {
    ...
    const data = useMemo(
        () => [
            {key: "1", name: "John Doe", data: "2024-10-24", category: "Mathematics", sum: "1000 USD", role: "Admin"},
            {key: "2", name: "Jane Smith", data: "2024-09-18", category: "Physics", sum: "1500 USD", role: "Teacher"},
            {key: "3", name: "Dr. Albert", data: "2024-07-12", category: "Chemistry", sum: "1200 USD", role: "Student"},
        ],
        []
    );
    ...
    const formData: IForm[] = [
        ...
        {label: t("Parol"), type: "password", name: "password", size: "large", span: 24, required: true},
        {
            label: t("Rol"),
            type: "select",
            name: "role",
    ...
    return (
        <div className="w-full bg-[#F9F9F9] min-h-[84vh]">
            ...
            <div
                className="mx-10 mt-5 w-[500px] h-[60px] rounded-[10px] shadow-md bg-white flex items-center justify-center gap-4 text-[20px]">
            ...
            <div className="mt-4">
                <Table
                    ...
                    dataSource={data}
                    size="large"
                    locale={{emptyText: <Empty description={<span>{t("Malumot topilmadi")}</span>}/>}}
                />
            </div>

            <Drawer title={t("Yangi guruh qo’shish")} onClose={onClose}
                    open={Boolean(query.add) || Boolean(query.edite)} width={530}>
