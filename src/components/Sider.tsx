...
export function Sider() {
    ...
    return (
        <AntdSlider
            theme="light"
            className="px-[30px] py-[20px] h-[100vh] relative overflow-hidden"
            width={300}
        >
            <div className="flex items-center justify-start gap-5">
                <img src="/SiderWomanImg.svg" alt="Woman Img"/>
                <div>
                    <Typography className="!m-0 !font-[700] text-[16px] font-[Poppins]">
                        Jamila Azimova
                    </Typography>
            ...
            <div className="mt-[40px] flex flex-col justify-between h-full">
                <div className="overflow-hidden">
                    {data.map((item, index) => (
                        <Link
                            to={item.link}
                            key={index}
                            className={`flex justify-start items-center gap-3 font-[600] font-[Source Sans Pro] px-[20px] py-[15px] 
            ${location.pathname.includes(item.link) ? 'bg-[#635AD9] text-white hover:text-white' : 'hover:text-black'}
